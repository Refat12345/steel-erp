/**
 * GET /api/billet-receipts — history permission gates operational-date browsing.
 */
...
vi.mock("@/lib/services/billet-receipt.service", () => ({
...
function req(query = "") {
  ...
  return new NextRequest(`http://localhost/api/billet-receipts${qs}`);
}
...
const operatorSession = {
  ...
  username: "scale1",
  role: "scale_operator",
  permissions: ["billet.receipt.view", "billet.receipt.weigh"],
};
...
const historySession = {
  ...
  username: "manager",
  role: "manager",
  permissions: ["billet.receipt.view", "billet.receipt.view_history"],
};

beforeEach(() => {
  vi.clearAllMocks();
  mockListReceipts.mockResolvedValue({
    data: [{ id: 1, receiptNumber: "R-26-0001" }],
    total: 1,
    page: 1,
    pageSize: 25,
  });
...
describe("GET /api/billet-receipts history gate", () => {
  ...
  it("returns 403 without billet.receipt.view", async () => {
  ...
  it("allows a past date when billet.receipt.view_history is present", async () => {
  ...
  it("does not treat truck.view_history as billet history access", async () => {
    mockGetApiSession.mockResolvedValue({
      ...operatorSession,
      permissions: ["billet.receipt.view", "truck.view_history"],
    });
    const res = await GET(req("operationalDate=2026-01-15"));
    expect(res.status).toBe(403);
    expect(mockListReceipts).not.toHaveBeenCalled();
