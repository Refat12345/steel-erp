...
مصمّم لمهندس برمجيات ما عنده خبرة سابقة بـ DevOps، مع وجود شخص تقني يفهم بالشبكات والسيرفرات.
...
2. [تقسيم المسؤوليات مع الشخص التقني](#2-تقسيم-المسؤوليات-مع-الشخص-التقني)
...
5. [المرحلة 1: استلام الـ VPS من الشخص التقني](#5-المرحلة-1-استلام-الـ-vps-من-الشخص-التقني)
...
## 2. تقسيم المسؤوليات مع الشخص التقني

### مبادئ حرجة (ناقشها معه من اليوم الأول)

| المبدأ | السبب |
|--------|-------|
| حساب Hetzner باسم الشركة/المدير، مش باسمه الشخصي | لو ترك الشغل، الشركة تضمن الوصول للـ VPS |
| أنت عندك `sudo` access كامل من اليوم الأول | النظام مسؤوليتك، لازم تقدر تتصرف بأي وقت |
| بيانات الدخول موثّقة بمكان الشركة (password manager تابع للشركة أو ظرف مغلق عند المدير) | استمرارية العمل |
| ملف `CHANGES.md` يوثّق كل تغيير على السيرفر | تجنّب "شو اللي صار بالضبط؟" لما يحصل مشكلة |

### مسؤوليات الشخص التقني (شبكات/سيرفرات)

- [ ] شراء VPS من Hetzner باسم الشركة (Plan: CX22, OS: Ubuntu 24.04 LTS).
- [ ] الإعداد الأولي للسيرفر وتأمينه (تفاصيل بالمرحلة 1).
- [ ] إضافة A record للـ subdomain (مثلاً `erp.company.com`) على DNS الشركة → IP الـ VPS.
- [ ] (موصى به) إضافة الدومين على Cloudflare مع تفعيل proxy (orange cloud).
- [ ] إعطاؤك SSH access لمستخدم `deploy` عنده `sudo`.

### مسؤولياتك أنت (مهندس البرمجيات)

- [ ] كل شي فوق layer الـ OS: Node.js, PM2, PostgreSQL, Nginx config, التطبيق.
- [ ] ملفات الأسرار (`.env.production`, database passwords, NEXTAUTH_SECRET).
- [ ] Migrations وتحديثات الـ schema.
- [ ] **النسخ الاحتياطية واختبار الاستعادة - هاد شغلك، لا تسلّمه لحدا.**
- [ ] Monitoring للتطبيق والـ deployment.
...
- [ ] IP الـ VPS (لما يشتريه الشخص التقني).
- [ ] اسم المستخدم SSH (الافتراضي: `deploy`).
...
## 5. المرحلة 1: استلام الـ VPS من الشخص التقني

### Checklist لاستلام VPS جاهز (راجعه معه)
...
راجعها مع الشخص التقني قبل ما تكمل. **لا تكمل على سيرفر غير مؤمّن.**
...
## $(date +%Y-%m-%d)
- Initial setup by [network admin name]
...
قبل ما تبدأ هالمرحلة، الشخص التقني لازم يكون عمل A record:
```
