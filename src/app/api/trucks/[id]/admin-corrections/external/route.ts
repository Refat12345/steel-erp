...
import { completedExternalCorrectionSchema } from "@/lib/validators/truck";
import { correctCompletedRoundExternal } from "@/lib/services/truck.service";
...
export async function PATCH(
  ...
  return withIdempotency(req, session.userId, parsed.text, async () => {
    const validated = completedExternalCorrectionSchema.safeParse(parsed.json);
    if (!validated.success) {
      return badRequest(validated.error.issues[0]?.message || "invalidData");
    }

    try {
      const { roundId, weightKg, reason, expectedVersion } = validated.data;
      const truck = await correctCompletedRoundExternal(
        truckId,
        roundId,
        weightKg,
        reason,
