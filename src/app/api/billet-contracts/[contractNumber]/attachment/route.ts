...
import { addContractAttachment } from "@/lib/services/billet-contract.service";
import { compressPdfBuffer } from "@/lib/server/pdf-compress";

const UPLOAD_DIR = path.join(process.cwd(), "uploads", "billet-contracts");
...
export async function POST(
  ...
  if (
    !hasPermission(session, "billet.contract.upload") &&
    !hasPermission(session, "billet.contract.edit")
  ) {
    return forbidden();
  }
  ...
  const relativePath = `uploads/billet-contracts/${storedName}`;

  await writeFile(filePath, storedBuffer);

  try {
    const attachment = await addContractAttachment(
      contractNumber,
